...
public class Game {
    ...
    public SendMessage giveBribe(User user, long chatID){
        ...
        if (gamer.getGamer_isArrested()) {
            ...
            if (gamer.getScore() >= 5) {
                ...
                text = "Пане " + user.firstName() + ", ти не маешь стiльки грошей щоб покурити ще раз.\n";
        }
    ...
    public SendMessage playGame(User user, Chat chat){
        ...
        if (gamer.getGamer_isArrested()){
            ...
            if(minutes < 180)
                return new SendMessage(chat.id(), "Сорі, пане " + user.firstName() + ", ти ще в ТРО"
                        ...
                        + "Вiдпустять через " + (180 - minutes) + " хвилин.");
            else
                gamer.setGamer_isArrested(false);
        }
        ...
        if (gamer.getGamer_lastTimeSmoked() != null) {
            ...
            if (minutes < 60)
                return new SendMessage(chat.id(), "Сорі, пане " + user.firstName() + ", ти награвся.\n"
                        + "Можна буде пограти через " + (60 - minutes) + " хвилин.");
        }
        ...
        if(percentage <= 8)
            return arrestGamer(gamer, user.firstName(), chat.id(), percentage);
        else if (percentage >= 60 && percentage <= 65)
            return gamerLostAllPoints(gamer, user.firstName(), chat.id(), percentage);
    ...
    public SendMessage topGamers(Chat chat){
        ...
        for(Gamer gamer : topGamers)
            text.append(i++).append(": ").append(gamer.getGamer_username() == null ? gamer.getGamer_id() : gamer.getGamer_username()).append(": ").append(gamer.getScore()).append(" грн.\n");

        return new SendMessage(chat.id(), text.toString());
    }

    private SendMessage gamerLostAllPoints(Gamer gamer, String userFirstName, long chatID, int percentage){
    ...
    private SendMessage arrestGamer(Gamer gamer, String userFirstName, long chatID, int percentage){
